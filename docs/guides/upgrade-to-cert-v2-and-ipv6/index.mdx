...
The Nebula v1.10 adds support for IPv6-addressed Nebula hosts. To support the
...
Firstly, we'll upgrade all hosts to the latest Nebula version >= v1.10. Next, we will issue backwards-compatible v1+v2 certificate bundles to each host. After updating each host's
