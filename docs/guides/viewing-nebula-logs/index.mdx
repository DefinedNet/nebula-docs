...
Another useful tool for viewing the logs can be [`Console.app`](https://support.apple.com/guide/console/welcome/mac) it
provides a nice UI for viewing logs in. To access the logs, select `File -> Open` from the menu and then press
`CMD+SHIFT+G` in the file picker, which will allow you to enter `/opt/homebrew/var/log/nebula.log` into the textbox.
