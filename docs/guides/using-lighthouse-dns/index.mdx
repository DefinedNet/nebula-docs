...
lighthouse. If enabled on a regular host, nebula will warn:
`DNS server refusing to run because this host is not a lighthouse.` and refuse to serve DNS on that non-lighthouse host.
