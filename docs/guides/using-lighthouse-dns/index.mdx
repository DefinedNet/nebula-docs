...
be able to query DNS on the lighthouse.
...
> The IP is the closest thing to a nebula "primary key" of sorts. An overlapping name/etc. is probably a mistake that
> can happen commonly, but only one host can actively have an IP (from the lighthouse perspective) at any moment. (note:
> if you give the same IP to two hosts, via valid certs, they will fight back and forth, but at the moment you query it
> will be correct.)

If I then host a server on `[::]:3000` on `alice-laptop` and set up a firewall rule that allows this to be accessed from
`bob-laptop`, I can request it with `curl` using the custom DNS resolver. (In a production environment, you may choose to configure the system resolver.)
