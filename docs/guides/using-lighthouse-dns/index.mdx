...
Additionally, add a [firewall rule](/docs/config/firewall) in your lighthouse config that enables UDP on port `53` for
all hosts you want to be able to query DNS on the lighthouse. For example to allow only hosts with the `role:Laptop`
group:
