...
`listen` sets the UDP port Nebula will use for sending/receiving traffic and for handshakes. For a lighthouse node, the
port should be defined, conventionally to `4242`, however using port 0 will dynamically assign a port and is recommended
for roaming nodes.
...
Sets the max number of packets to pull from the kernel for each syscall (under systems that support `recvmmsg`). Does
not support reload
