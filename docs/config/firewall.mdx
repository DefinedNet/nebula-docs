...
- `local_cidr`: a local CIDR, `0.0.0.0/0` is any. This restricts which destination IP addresses, when using
  ...
  well as any IP addresses it routes to. It is a good idea to specify this setting on any box that is configured as an
  unsafe_routes router. (e.g. Use the node's Nebula IP with a `/32` mask to allow inbound access only to the node
  itself.)
