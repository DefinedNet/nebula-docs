...
function buildPageObj(slug: string, langCode: string, allPages: any) {
  ...
  let page = allPages.find(
    (p) => getSlugFromFilename(p.file) === slug && getLanguageFromFilename(p.file) === langCode
  );
  // If there wasn't a translated version, fall back to default.
  if (!page) {
    page = allPages.find(
      (p) =>
        getSlugFromFilename(p.file) === slug && getLanguageFromFilename(p.file) === DEFAULT_LOCALE
    );
  }
  return {
    title: page.frontmatter.title,
    href: `${SITE.root}/${langCode}/${page.frontmatter.slug}/`,
  };
}
